            ...
            <table class="text-gray-600 min-w-full divide-y divide-gray-200 table-fixed">
                <thead class="border-b border-gray-300 bg-gray-200">
                    <tr class="px-4 py-2 text-center text-sm font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap">
                        ...
                        <th scope="col" wire:click="order('business_name')" class="w-1/4 px-4 py-2 cursor-pointer">
                            <i class="fas fa-sort mr-2"></i>
                            Razón social
                        </th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($suppliers as $supplier)
                        <tr class="bg-gray-50">
                            ...
                            <td class="px-6 py-3 whitespace-nowrap text-center">
                                <p class="text-sm uppercase">
                                    {{ $supplier->purchases->where('is_active', true)->count() }} compras
                                </p>
                            </td>
                            <td class="px-6 py-3 whitespace-nowrap text-center">
                                <p class="text-sm uppercase">
                                    @if ($supplier->purchases->count() > 0)
                                        {{ Date::parse($supplier->purchases->last()->date)->format('d/m/Y') }}
                                    @else
                                        Sin compras
                                    @endif
